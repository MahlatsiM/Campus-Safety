...
MAX_DAILY_REPORTS = int(os.getenv('MAX_DAILY_REPORTS', 3))

# Check if API keys are configured
if not GEOAPIFY_API_KEY:
    st.error("❌ GEOAPIFY_API_KEY not configured. Check your .env file.")
    st.info("Get a free API key at: https://www.geoapify.com/")
    st.stop()
...
def get_route_geometry(start_lat, start_lon, end_lat, end_lon, debug=False):
    ...
    if not OPENROUTESERVICE_API_KEY:
        if debug:
            st.warning("⚠️ OpenRouteService API key not configured. Using straight line fallback.")
    ...
    try:
        ...
        if e.response.status_code == 403:
            ...
            **How to fix:**
            ...
            6. Add it to your .env file: OPENROUTESERVICE_API_KEY=your_token_here
...
if st.session_state.get("authentication_status"):
    ...
    try:
        ...
        st.info(f"📊 Reports submitted today: **{today_count}/{MAX_DAILY_REPORTS}**")

        if today_count >= MAX_DAILY_REPORTS:
            st.warning(f"⚠️ You have reached the maximum of {MAX_DAILY_REPORTS} reports for today. Please try again tomorrow.")
        else:
